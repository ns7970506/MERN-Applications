...
const Checkout = () => {
  ...
  const orderItems = cartItems.map(({ productId, qty, price }) => ({
    productId,
    quantity: qty,
    price,
  }));
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (
        ...
        window.confirm("Payment failed. Place this as a local test order using Bypass Mode?")
      ) {
        return bypassPayment();
      }
  ...
  const bypassPayment = async () => {
    ...
    try {
      ...
      if (saveOrderRes.ok) {
        ...
        alert(result.message || "Test order could not be saved");
      }
    } catch (error) {
      console.error(error);
