  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        if (res.ok) {
          // API data can contain null for a missing/invalid revenue value.
          // Keep dashboard display values numeric before calling toFixed().
          setStats({ ...data, totalRevenue: Number(data.totalRevenue) || 0 });
        } else {
          if (res.status === 401) {
            navigate("/login");
          }
